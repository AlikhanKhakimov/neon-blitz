...
public class TirTête1 : MonoBehaviour
{
    Poupée1 poupée; // Référence à la classe Poupée pour accéder à ses méthodes et variables
    public ParticleSystem têteSang; // Système de particules pour représenter du sang lors d'un tir à la tête

    void Start()
    {
        poupée = transform.root.gameObject.GetComponent<Poupée1>(); // Obtient la référence au script Poupée attaché à la racine de cet objet
    }
